...
import RoomReportModal from "../../components/common/modal/RoomReportModal";
...
const LiveRoomPage = () => {
  ...
  const showToast = (message: string, type: "success" | "error" = "success", position?: "top-center" | "bottom-right") => {
  ...
  // 신고 처리 함수
  const handleReportRoom = async (reportText: string) => {
    if (!room || !roomId) return;

    try {
      await createReport({
        reportedId: room.hostId,
        contentId: Number(roomId),
        reportedContent: room.title,
        reportType: ReportType.ROOM,
        reportReason: reportText,
      });

      setIsReportModalOpen(false);
      showToast("신고가 접수되었습니다", "success", "top-center");
      
    } catch (error) {
      const err = error as any;
      console.error("방 신고 실패:", err.response?.data);
      ...
      if (
        errorData?.code === 'DUPLICATE_REPORT' ||
        (status === 400 && errorData?.message?.includes('이미 신고한'))
      ) {
        showToast("이미 신고한 방입니다", "error", "top-center");
        setIsReportModalOpen(false);
        return;
      }
      ...
      // 권한 에러
      if (status === 401) {
        showToast("로그인이 필요합니다", "error", "top-center");
        setIsReportModalOpen(false);
        return;
      }
      
      // 기타 에러
      showToast("신고 처리 중 오류가 발생했습니다", "error", "top-center");
    }
  };
    ...
    const loadRoom = async () => {
      try {
        ...
        if (data?.locked === true) {
          ...
          setEntryQuestion(question || DEFAULT_QUIZ_PROMPT);
          setIsQuizModalOpen(true);
          return;
        }
        ...
        <>
          ...
          {room && (
            <LiveHeader
              ...
              onReport={!isHost ? () => setIsReportModalOpen(true) : undefined}
            />
          )}
