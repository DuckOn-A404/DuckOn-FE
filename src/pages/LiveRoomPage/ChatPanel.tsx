...
const ChatPanel = ({
  ...
  // 새로고침 후 재입장 시 sessionStorage에 이전 세션의 guestId가 남아있을 수 있기 때문에
  // 서버는 재입장마다 새로운 guestId를 할당하므로, 항상 null로 시작하고, 메시지 수신 후 서버 할당 ID를 학습하도록 한다.
  ...
  // 게스트가 방금 보낸 메시지를 임시로 저장하는 큐. guestId를 아직 모르는 상태에서도 '내가 보낸 메시지'라고 판단하기 위해 사용
  const pendingGuestSendsRef = useRef<Array<{ content: String; at: number }>>([]);
  ...
  useEffect(() => {
    ...
    if (addedCount > 0) {
      ...
      // myUser?.userId와 guestId를 합쳐서 판단하면, stale guestId가 있을 때 !myIdNow 조건이 false가 된다.
      // 학습 분기 자체를 타지 않게 된다. 로그인 ID만으로 분리해야 비로그인 상태에서 항상 학습 분기를 탈 수 있다.
      const myLoggedInId = String(myUser?.userId ?? "");
      ...
      if (myLoggedInId && lastSenderId && myLoggedInId === lastSenderId) {
        fromMe = true;
      } else if (
        // !myIdNow &&
        !myLoggedInId &&
        ...
        pendingGuestSendsRef.current = pendingGuestSendsRef.current.filter((p) => {
          if (p.content !== last.content) return true;
          const lastMs =
            typeof (last as any).sentAt === "number"
              ? (last as any).sentAt
              : new Date((last as any).sentAt).getTime();
          return Math.abs(lastMs - p.at) > 2000;
        });
        try {
          if (typeof window !== "undefined") {
  ...
  const handleSendMessage = () => {
    ...
    // 게스트가 보내면 "내가 보낸 기록"을 큐에 넣기 -> 이후 서버 브로드 캐스트 메시지와 비교하여 내 메시지 여부를 판단하는 데 사용
    // guestId는 나중에 학습되는데 그 전에 이미 메시지가 화면에 렌더링 되기에 내가 보낸 메시지라는 증거를 남겨둠
    if (!isLoggedIn) {
      pendingGuestSendsRef.current.push({ content: v, at: sentAt });
      if (pendingGuestSendsRef.current.length > 20){  // 최근 20개만 유지
        pendingGuestSendsRef.current.shift();
      }
    }
  ...
  // 로그인 유저와 게스트의 ID 출처가 다르므로 분리해서 처리하기.
  // 로그인 유저는 myUser.userId이고 게스트는 메시지 수신 후 학습된 guestId로 서버 할당 임시 ID이다.
  // ?? 체이닝으로 합치면 로그인 상태 전환 시 간섭이 발생할 수 있기에 분리해서 처리!
  const myId = myUser?.userId ? String(myUser.userId) : guestId ?? "";
      ...
      <div className="relative flex flex-col h-full bg-gray-800 text-white">
        ...
        >
          {visibleMessages.map((msg, index) => {
            ...
            const sentAtRaw = (msg as any).sentAt;
            const sentAtMs =
              typeof sentAtRaw === "number" ? sentAtRaw : new Date(sentAtRaw).getTime();
            // guestId가 아직 학습되지 않은 상태에서도 내가 방금 보낸 메시지라면 오른쪽에 정렬되도록 optimistic 판단을 추가
            // 방금 내가 보낸 메시지와 내용과 시각이 비슷하면 처음부터 오른쪽으로 렌더링
            const isOptimisticGuestMine =
              !isLoggedIn &&
              !myId && // guestId 아직 학습 전
              pendingGuestSendsRef.current.some((p) => {
                if (p.content !== (msg.content ?? "")) return false;
                return Math.abs(sentAtMs - p.at) <= 2000; // 2초 허용
              });

            const isMyMessage =
              (senderId !== "" && myId !== "" && senderId === myId) || isOptimisticGuestMine;
